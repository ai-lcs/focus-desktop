...
# 整段替换：MakeTabButton 到 ActivateTab 结束（当前文件里的旧块）
...
assert start >= 0 and end > start, f"anchors not found: start={start} end={end}"
end += len(end_anchor)

new_block = '''    private WpfButton MakeTabButton(string id, string title, bool closable)
    {
        // 浏览器式 Tab：圆角上缘 + 激活态顶部青绿下划线 + 可关闭 ✕（Dragablz/Chrome 视觉语言）
...
# 删掉旧 BuildTabBar（新块里已含）——旧块在 end 之后紧跟 "private void BuildTabBar()"
old_btb = '''
    private void BuildTabBar()
    {
        TabBar.Children.Clear();
        ...
        var home = MakeTabButton("home", "\\U0001F3E0 首页");
        _tabButtons["home"] = home;
        TabBar.Children.Add(home);

        var files = MakeTabButton("files", "\\U0001F4C2 学习文件");
...
'''
# 源文件里 emoji 是字面字符，不是转义——用正则容错
m = re.search(r'\n    private void BuildTabBar\(\)\n    \{\n        TabBar\.Children\.Clear\(\);.*?\n    \}\n', src[src.find(new_block):], re.S)
if m:
    seg = src.find(new_block)
    abs_start = seg + m.start()
    abs_end = seg + m.end()
    src = src[:abs_start] + '\n' + src[abs_end:]
    print("old BuildTabBar removed")

open(p, 'w', encoding='utf-8').write(src)
print("tab system rewritten:", len(new_block), "chars")
