...
- **硬性专注**（v0.4.0）：番茄钟卡片上的开关。开启后退出弹窗输入框禁用（无法退出）、番茄钟不可重置，唯一出口=当前专注段跑完自动解除；预览模式仅演示；
- **网页 Tab 秒切**（v0.5.0）：所有网页常驻不隐藏，切换只改前后层次——无白闪、无黑屏（像素实测 ≤100ms 出画面）；被遮挡的 B 站视频切走也不中断；
- **Tab 浏览器化**（v0.5.1）：浏览器式标签栏，「+」紧跟最后一个 Tab；Tab 只显示站点短名（固定 60 宽）；
- **番茄钟重制**（v0.5.4）：iOS 式硬性专注滑块开关（开始前可关、开始后锁定、跑完自动关，开启即拨 30 分钟）+ Grid 分段时长选择（等宽等距）+ 总分钟数时间格式（60 分钟显示 60:00 而非 00:00）+ 进度弧实时缺口；
